...
Any for of Statistical Learning algorithminvolves optimization. Optimization refers to the task of either minimizing or maximizing some function $f(\mathbf{\beta})$ by altering $\beta$. We usually phrase most optimization problems in terms of minimizing $f(\beta)$. For the case of parametric learning models (we place distributional assumptions on the target/response $y_i$, such that they are drawn independently from  some probability distribution $p_{model}(y, \theta)$), where $\theta$ is unknown.  we can also use the process of Maximum Likelihood to find model coefficients. Under this approach, we have the following optimization problem:

$$\beta^* =arg max(l(y_i))$$ 

where $l(y_i)$ is the likelihood function. 

<div class="alert alert-info">
  <strong>Why Maximum Likelihood?</strong> There are several reasons why the process of Maximum Likelihood is preferential:
...
and then log-likelihood (since the product of exponential terms is the exponential of the sum). 

$$log(L(f(y_i))) = l(f(y_i)) = \sum_{i = 1}^n \frac{1}{a(\phi)}(y_i \theta_i - b(\theta_i)) + c(y_i, \phi)$$

We wish to maximize this log-likelihood, hence we can differentiate, equate to zero and solve for $\beta_j$. We can also ensure the second derivative evaluated at $\beta_j$ is negative, therefore we have maximized (and not minimized) the log-likelihood. Via the Chain Rule we have:
...
<div class="alert alert-info">
  <strong>Chain Rule</strong> Chain Rule is often used in Optimization problems. Here we can utilize chain rule by recognizing that, as seen in _3.1_, the likeihood is a function of the location parameter of the distribution, $\theta$, which in turn is a function of the mean $\mu$. Via the link function, we have that $g(\mu_i) = \eta_i$ and via the linear predictor, $\eta_i = \beta_0 + \sum_{i = 1}^{p}\beta_p x_{i, p}$. 
</div>

This is also known as the Score function, since it tells us how sensitive the model is to changes in $\beta$ at a given value of $\beta$.
...
Each of these individual partial derivatives can be solved:
...
since $g(\mu_i) = \eta_i \implies \frac{\partial \eta_i}{\partial \mu_i} = g'(\mu_i)$
...
Effectively, we need to find coefficients $\beta_j$ (which for each observation $y_i$ affect our prediction of $\mu_$, $g'(\mu_i)$ and $V(\mu_i)$ via our distribtuional assumptions for the relationship between the mean and the variance), such that summing these terms over all observations gives 0. To solve this, we use the Newton - Raphson Method
...
Given that $\frac{\partial l}{\beta_j} = \grad_{\beta} l = \frac{(y_i - \mu_i)}{a(\phi)} \frac{x_{i,j}}{V(\mu_i)}\frac{1}{g'(\mu_i)}$, the Hessian is then:

__Equation 4__

$$\grad^{2}_{\beta} = \sum_{i = 1}^n \frac{x_{i,j}}{a(\phi)} \left( (y_i - \mu_i)' \frac{1}{g'(\mu_i)} \frac{1}{V(\mu_i)} + (y_i - \mu_i) \left( \frac{1}{g'(\mu_i)} \frac{1}{V(\mu_i)} \right)' \right)$$
...
<div class="alert alert-info">
  <strong>Fisher Scoring</strong> $$\beta_{t+1} = \beta_t = J^{-1} \grad l$$ where $J = \mathop{\mathbb{E}}[- \grad^2 l]$, the expected value of the negative Hessian.
</div>
...
$$\mathbf{\beta}_{t+1} = \mathbf{\beta_t} = \mathbf{J}^{-1} \grad_{\beta} l (\beta_t)$$

We can rewrite $\grad_{\beta}l = \sum_{i = 1}^n \frac{y_i - \mu_i}{a(\phi)} frac{1}{a(\phi)} \frac{x_{i,j}}{V(\mu_i g'(\mu_i))}$ as  $\mathbf{X}^T \mathbf{D} \mathbf{V}^{-1} (y - \mu)$ where:
